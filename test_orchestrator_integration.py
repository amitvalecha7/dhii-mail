#!/usr/bin/env python3
"""
Test script for A2UI Orchestrator with State Machine integration
"""
...
def test_orchestrator_integration():
    ...
    print(f"Email inbox navigation: {'✅ Success' if 'ui_update' in result else '❌ Failed'}")
    ...
    print(f"Calendar navigation: {'✅ Success' if 'ui_update' in result else '❌ Failed'}")
    print(f"New state: {orchestrator.state_machine.get_current_state().value}")
    
    # Test invalid navigation
    print("\n--- Testing Invalid Navigation ---")
    result = orchestrator.handle_action("navigate_analytics")  # Should fail from calendar
    print(f"Analytics navigation from calendar: {'✅ Success' if 'ui_update' in result else '❌ Failed (expected)'}")
