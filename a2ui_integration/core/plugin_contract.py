"""
Framework 2.0: Core Plugin Contracts & Types
...
# Standard plugin registration function signature
def register_plugin(kernel_api: Dict[str, Any]) -> PluginInterface:
    """
    Standard registration function - ALL plugins must implement this
    
    Args:
        kernel_api: Dictionary with kernel API functions
        
    Returns:
        PluginInterface instance
        
    Example:
        def register_plugin(kernel_api):
            plugin = MyPlugin()
            plugin.initialize(kernel_api)
            return plugin
    """
    raise NotImplementedError("Plugins must implement register_plugin function")